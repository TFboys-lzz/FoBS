...
class DeformLaplace(nn.Module):
    def __init__(self, inc, outc,cfg, kernel_size=3, padding=1, stride=1, bias=None, modulation=False):
        """
        Args:
            modulation (bool, optional): If True, Modulated Defomable Convolution (Deformable ConvNets v2).
        """
        super(DeformLaplace, self).__init__()
        ...
        lap = [[-1.0 / 8.0, -1.0 / 8.0, -1.0 / 8.0], [-1.0 / 8.0, 1, -1.0 / 8.0],
               [-1.0 / 8.0, -1.0 / 8.0, -1.0 / 8.0]]

        self.deformable_kernel =torch.tensor(lap, dtype=torch.float32).unsqueeze(0).expand(1, hidden_channels, 3, 3).to(
    ...
    def forward(self, x):
        ...
        out = F.conv2d(x_offset, self.deformable_kernel, stride=3, padding=1)
...
class deeplabv3plus(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.beta2 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        self.beta2.data.fill_(1.0)

        #self.reluLayer = nn.ReLU()


        self.feature_refine_conv = DeformLaplace(inc=cfg.MODEL_ASPP_OUTDIM + cfg.MODEL_SHORTCUT_DIM, outc=cfg.MODEL_ASPP_OUTDIM + cfg.MODEL_SHORTCUT_DIM,cfg=cfg)
    ...
    def forward(self, x, y_true=None, y=None, margin=None, feature_cat=None, phase='orig'):
        ...
        # training
            ...
            feature_aspp = self.upsample_sub(feature_aspp)
            feature_shallow = self.shortcut_conv(layers[0])
            feature_cat_1 = torch.cat([feature_aspp, feature_shallow],
                                      1)  # print(feature_cat.shape)  # [16, 304, 64, 64]
            ...
            att_guide_1 = torch.where(temp_guide > margin, temp_guide,
                                      zeros_mat)  # print(att_guide.shape)  # [16, 64, 64]
            ...
            offset_1, _ = self.feature_refine_conv(feature_cat_1)

            # refine_feature_cat_2 = feature_cat_1 - offset_1 * (torch.exp(self.para * torch.unsqueeze(att_guide_1, 1))-1)
            # self.beta1 = self.sigmoidLayer(self.beta1)  # (0,1)
            # self.beta2 = torch.exp(self.beta2)  # >0
            # self.beta2 = self.reluLayer(self.beta2)
            # bt2 = torch.exp(self.beta2)
            # bt2 = self.sigmoidLayer(self.beta2)
            # coe = 1.5 * ((1-torch.exp(-1*bt2*torch.unsqueeze(att_guide_1, 1)))/(1+torch.exp(-1*bt2*torch.unsqueeze(att_guide_1, 1))))
            # bt1 = torch.exp(self.beta1)
            # coe = 1.5 * ((1 - torch.exp(-1 * self.beta2 * torch.unsqueeze(att_guide_1, 1))) / (1 + torch.exp(-1 * self.beta2 * torch.unsqueeze(att_guide_1, 1))))
            coe = self.beta1 * ((1 - torch.exp(-1 * self.beta2 * torch.unsqueeze(att_guide_1, 1))) / (
                        1 + torch.exp(-1 * self.beta2 * torch.unsqueeze(att_guide_1, 1))))

            # coe = self.sigmoidLayer(coe)
            # print('f1:',feature_cat_1.shape)
            # print('off1:',offset_1.shape)
            # print('coe:',coe.shape)
            # print('offcoe:', (offset_1 * coe).shape)
            ...
            # return feature_cat_1, result_1, refine_feature_cat_2, refine_result_2, self.para, avg_att
            return feature_cat_1, result_1, refine_feature_cat_2, refine_result_2, temp_guide, self.beta2, coe, offset_1 * coe