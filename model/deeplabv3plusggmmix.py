...
class deeplabv3plusggmmix(nn.Module):
    def __init__(self, cfg):
        super(deeplabv3plusggmmix, self).__init__()
        self.backbone = None
        ...
        '''
        self.refine_upsample4 = nn.UpsamplingBilinear2d(scale_factor=4)
        self.refine_cat_conv = nn.Sequential(
        ...
        self.refine_cls_conv = nn.Conv2d(cfg.MODEL_ASPP_OUTDIM, cfg.MODEL_NUM_CLASSES, 1, 1, padding=0)
        '''

        # method 1
        #self.para = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        #self.para.data.fill_(1.0)
        #self.para = torch.tensor([1.0])

        # method 2
        #self.beta1 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        #self.beta1.data.fill_(0.0)
    ...
    def forward(self, x, y_true=None, y=None, margin=None, feature_cat=None, phase='orig'):
        ...
        # training
            ...
            if phase=='orig':
                ...
                feature_aspp = self.upsample_sub(feature_aspp)
                feature_shallow = self.shortcut_conv(layers[0])
                feature_cat = torch.cat([feature_aspp, feature_shallow], 1)  # print(feature_cat.shape)  # [16, 304, 64, 64]

            ############ path one ori result ###############
            result_1 = self.cat_conv(feature_cat)
            result_temp_1 = self.cls_conv(result_1)
            result = self.upsample4(result_temp_1)
            ...
            offset_1, _ = self.offset_laplas_conv(feature_cat)
            #refine_feature_cat_2 = feature_cat - offset_1 * (torch.exp(self.para * torch.unsqueeze(att_guide_1, 1))-1)
            #  # (0,1)
            ...
            refine_feature_cat_2 = feature_cat - offset_1 * coe
            ############ refine path2 result ###############
            ...
            #return feature_cat, result_1, refine_feature_cat_2, refine_result_2, self.para, avg_att
            return feature_cat, result, refine_feature_cat_2, refine_result_2, avg_att, self.beta2, coe, result_temp_1